...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (kid_id) {
      ...
      let hiddenCats = [];
      if (hiddenResult.rows.length > 0 && hiddenResult.rows[0].hidden_activities_categories) {
        hiddenCats = hiddenResult.rows[0].hidden_activities_categories.split(',').map(cat => cat.trim());
      }
