...
try {
    ...
    if ($kidId) {
        $hiddenCategories = fetchOne(
            "SELECT hidden_activities_categories FROM children WHERE id = :kid_id",
            ['kid_id' => $kidId]
        );

        $hiddenCats = [];
        if ($hiddenCategories && $hiddenCategories['hidden_activities_categories']) {
            $hiddenCats = explode(',', $hiddenCategories['hidden_activities_categories']);
    ...
    $sql = "SELECT * FROM activities $condition ORDER BY RAND() $limitClause";
