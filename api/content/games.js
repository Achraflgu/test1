  ...
  try {
    ...
    if (kid_id) {
      const hiddenCategories = await client.query(
        'SELECT hidden_games_categories FROM children WHERE id = $1',
        [kid_id]
      );

      let hiddenCats = [];
      if (hiddenCategories.rows.length > 0 && hiddenCategories.rows[0].hidden_games_categories) {
        hiddenCats = hiddenCategories.rows[0].hidden_games_categories.split(',').map(cat => cat.trim());
      }
    ...
    const games = await client.query(
      `SELECT * FROM games ${whereClause} ORDER BY RAND() LIMIT ${limitParam}`,
      params
    );
