...
try {
    ...
    if ($kidId) {
        $hiddenCategories = fetchOne(
            "SELECT hidden_games_categories FROM children WHERE id = :kid_id",
            ['kid_id' => $kidId]
        );

        $hiddenCats = [];
        if ($hiddenCategories && $hiddenCategories['hidden_games_categories']) {
            $hiddenCats = explode(',', $hiddenCategories['hidden_games_categories']);
    ...
    $sql = "SELECT * FROM games $condition ORDER BY RAND() $limitClause";
